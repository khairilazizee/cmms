...
defined( '_UMPORTAL_ADMIN' ) or die( 'Akses tidak dibenarkan !' );
 ...
 $susunan=$max;	 
 ?>
<form name="frmmenu" method="post" action="admin.php?module=menu&task=simpan">
<table id="form_table_outer" width="60%">
  <tr><td class="form_table_header">Tambah Menu Pengguna</td></tr>
  <tr><td>
<table id="form_table_inner" width="100%" border="0" bgcolor="#EEF3FF" cellpadding="2" cellspacing="0">
	  ...
	  $jenismenu[1]="pautan";
	  $jenismenu[2]="modul";
	  $jenismenu[3]="maklumat";

	  $labeljenis[0]="Menu";
	  $labeljenis[1]="Pautan";
	  $labeljenis[2]="Modul";
	  $labeljenis[3]="Maklumat";
	  
	  echo "<td>Jenis</td>";
      echo "<td colspan=\"3\"><select name=\"txt_jenis\" onChange=\"document.frmmenu.action='admin.php?module=menu&task=tambah';document.frmmenu.submit();\">";
	  for ($idx=0;$idx<4;$idx++){
	...
	?>
      ...
      if (!isset($jenis))
	     $jenis="menu";
	  if ($jenis<>"menu"){
        echo "<tr><td>Kategori Menu</td><td colspan=\"3\"><select name=\"txt_menu\" onChange=\"document.frmmenu.action='admin.php?module=menu&task=tambah';document.frmmenu.submit();\" >";
	    $query = "SELECT id,title FROM menu where type='menu' and admin='0' order by menupos";
        $result = sql_query($query,$dbi);
	    $num_rows = sql_num_rows($result);
	  ...
	  if ($jenis=="maklumat"){
        echo "<tr><td>Kategori Maklumat</td><td colspan=\"3\"><select name=\"txt_kategori\" onChange=\"document.frmmenu.txt_tajuk.value=this.options[this.selectedIndex].text;  document.frmmenu.txt_pautan.value='mainpage.php?module=Maklumat&kategori='+this.value;\">";
	   ...
	   if($num_rows > 0) {
	      while($data=sql_fetch_array($result,$dbi)){ 
		     ...
		     $tajuk = $data["title"];
			 if ($c_id==$category)
	           echo "<option selected value=\"$c_id\">$tajuk</option>";
			 else
	           echo "<option value=\"$c_id\">$tajuk</option>";
		  }
	   }
        echo "</select></td></tr>";
	 } //jenis=="content   
		 ...
		 if ($jenis=="modul") { 
            echo "<tr><td>Modul</td>";
			echo "<td colspan=\"3\"><select name=\"txt_modul\" onChange=\"document.frmmenu.txt_pautan.value='mainpage.php?module='+document.frmmenu.txt_modul.value;\">";

	       $query = "SELECT admin,name,title FROM modules where admin='0' order by admin,title ";
           $result = sql_query($query,$dbi);
	       $num_rows = sql_num_rows($result);
	       ...
	       if($num_rows > 0) {
	          ...
	          while($data=sql_fetch_array($result,$dbi)){ 
				...
				if ($cnt_m==1){
				  if(!isset($pautan))
				     $pautan="mainpage.php?module=$m_name";
				}				
				if ($m_name==$module)
	   ...
	   ?>
          ...
          <tr> 
            <td colspan="2" > <input type="hidden" name="txt_targetwindow" value="<? echo $targetwindow; ?>"> 
              ...
              <input name="Simpan" type="submit" value="Simpan"> <input name="Kembali" type="Button" value="Kembali" onclick="location.href='admin.php?module=menu';"> 
            </td>
          </tr>
        </table></td></tr></table>
</form>
...
$path="images/menu/";
 ...
 while($func=$blocksdir->read()) {
	    if($func!= ".." and $func!=".") 
			if(!is_dir($path.$func)){
                   echo "<a href=\"#\" onClick=\"document.frmmenu.txt_imej.value='$func';hidediv();\"><img border=\"0\" src=\"$path$func\"></a>";   
			}
  } //while				    
 echo "</div>";
 echo "</div>";
