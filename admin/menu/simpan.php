...
defined( '_UMPORTAL_ADMIN' ) or die( 'Akses tidak dibenarkan !' );
 ...
 function susun($ordering,$old_ordering,$flg,$jenis,$menu)
 {
  ...
  if ($flg==0){
    if ($jenis=="menu")
       $query = "SELECT id,menupos FROM menu where  admin='0' and type='$jenis' and menupos <= $ordering and menupos > $old_ordering order by menupos ";
	else
       $query = "SELECT id,menupos FROM menu where admin='0' and parent='$menu' and menupos <= $ordering and menupos > $old_ordering order by menupos ";
  }	
  else {
    if ($jenis=="menu")
       $query = "SELECT id,menupos FROM menu where admin='0' and type='$jenis' and  menupos >= $ordering and menupos < $old_ordering order by menupos "; 
 ...
 //echo $query."<br>";
  ...
  if ($dbtrans=="0") //insert
     $qry="insert into menu(parent,title,link,`type`,category,target_window,active,menupos,admin,module,picture,description)
	       values('$menu','$tajuk','$pautan','$jenis',$category,'$targetwindow','$aktif',$ordering,'0','$module','$image','$catatan')"; 
  else if ($dbtrans=="1") { //update
     $qry="update menu set parent='$menu',title='$tajuk',link='$pautan',`type`='$jenis',
	       target_window='$targetwindow',active='$aktif',menupos='$ordering',admin='0',category=$category,module='$module',picture='$image',description='$catatan' where id=$id";
